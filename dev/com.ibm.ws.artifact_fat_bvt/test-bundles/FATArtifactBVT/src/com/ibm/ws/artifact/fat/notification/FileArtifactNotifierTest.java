 ...
 *
 */
public class FileArtifactNotifierTest extends AbstractArtifactNotificationTest {
        ...
        if (base != null) {
            ...
            if (mkdirsOk) {
                ...
                if (rootContainer == null) {
                    out.println("FAIL: unable to create ArtifactContainer for root File in File Artifact Notifier testcase. (test case issue, not code issue)");
                    return false;
                }
                this.overlayContainer = ocf.createOverlay(OverlayContainer.class, rootContainer);
                if (overlayContainer == null) {
                    out.println("FAIL: unable to create unable to create OverlayContainer for root File in for File Artifact Notifier testcase. (test case issue, not code issue)");
                    return false;
                }
    ...
    public boolean testNoNotificationsAfterRemovalOfListener(PrintWriter out) {
        ...
        try {
            ...
            if (!waitForInvoked(invoked, 0, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was invoked unexpectedly");
                pass = false;
            }
            //reset the sets
            invocable.add(test);
            ...
            if (!waitForInvoked(invoked, 2, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listeners were not invoked as expected");
                pass = false;
            }
            ...
            if (!waitForInvoked(invoked, 1, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: remaining listener was not invoked as expected invoked :" + invoked + " invocable :" + invocable);
                pass = false;
            }
    ...
    public boolean runMultipleOverlappingNonRootListenerTest(PrintWriter out) {
        ...
        try {
            ...
            if (!waitForInvoked(invoked, 1, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was not invoked as expected [i] invocable:" + invocable + " invoked:" + invoked);
                pass = false;
            }
            ...
            if (!waitForInvoked(invoked, 2, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was not invoked as expected [ii] invocable:" + invocable + " invoked:" + invoked);
                pass = false;
            }
            ...
            if (!waitForInvoked(invoked, 0, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was invoked as unexpectedly [iii] invocable:" + invocable + " invoked:" + invoked);
                pass = false;
            }
    ...
    public boolean testAddRemoveOfExistingViaOverlayBecomesModified(PrintWriter out) {
        ...
        try {
            ...
            if (!verifyCounts(testone, new String[] {}, new String[] {}, new String[] { "/one" }, out)) {
    ...
    public boolean testBaseAndOverlaidContentWithMaskUnmask(PrintWriter out) {
        ...
        try {
            ...
            if (!verifyCounts(testone, new String[] {}, new String[] { "/one" }, new String[] {}, out)) {
                out.println("FAIL: mask of existing did not become removed. " + testone);
                pass = false;
            } else if (!testone.getRemoved().get(0).getPaths().contains("/one")) {
                out.println("FAIL: Incorrect path in notification, expected /one in removed. " + testone);
                pass = false;
            }
            ...
            if (!verifyCounts(testone, new String[] { "/one" }, new String[] {}, new String[] {}, out)) {
                out.println("FAIL: unmask of existing did not become added. " + testone);
                pass = false;
            } else if (!testone.getAdded().get(0).getPaths().contains("/one")) {
                out.println("FAIL: Incorrect path in notification, expected /one in added. " + testone);
                pass = false;
            }
            ...
            if (!verifyCounts(testone, new String[] {}, new String[] { "/other" }, new String[] {}, out)) {
                out.println("FAIL: mask of existing did not become removed. " + testone);
                pass = false;
            } else if (!testone.getRemoved().get(0).getPaths().contains("/other")) {
                out.println("FAIL: Incorrect path in notification, expected /other in removed. " + testone);
                pass = false;
            }
            ...
            if (!verifyCounts(testone, new String[] { "/other" }, new String[] {}, new String[] {}, out)) {
                out.println("FAIL: unmask of existing did not become added. " + testone);
                pass = false;
            } else if (!testone.getAdded().get(0).getPaths().contains("/other")) {
                out.println("FAIL: Incorrect path in notification, expected /other in added. " + testone);
                pass = false;
            }
    ...
    public boolean testNestedChangeCausesEntryChange(PrintWriter out) {
        ...
        out.println("Nested Causes Entry Change (No-Op for File)");
        return true;
    }
    ...
    public boolean testNonExistingUnderOverlay(PrintWriter out) {
        ...
        try {
            ...
            if (!verifyCounts(testone, new String[] { "/one" }, new String[] {}, new String[] {}, out)) {
                out.println("FAIL: create did not become added for overlay " + testone);
                pass = false;
            }
            ...
            if (!verifyCounts(testone, new String[] {}, new String[] { "/one" }, new String[] {}, out)) {
                out.println("FAIL: delete did not become removed for overlay " + testone);
                pass = false;
            }
    ...
    public boolean testNonRecursiveNotificationAtNonRoot(PrintWriter out) {
        ...
        try {
            ...
            if (!waitForInvoked(invoked, 1, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was not invoked once as expected " + invoked);
                pass = false;
            }
            ...
            if (!waitForInvoked(invoked, 1, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was invoked when not expected " + invoked);
                pass = false;
            }
    ...
    public boolean testNonRecursiveNotificationAtRoot(PrintWriter out) {
        ...
        try {
            ...
            if (!waitForInvoked(invoked, 1, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was not invoked as expected " + testone);
                pass = false;
            }
            ...
            if (!removeFile(dataDir)) {
                out.println("FAIL: unable to remove base data dir to test root deletion (test issue, not code issue)");
                pass = false;
            }
            if (!waitForInvoked(invoked, 1, INTERVAL, SLEEP_DELAY)) {
                out.println("FAIL: listener was not invoked as expected " + testone);
            ...
            if (!found) {
                out.println("FAIL: delete of root did not cause / notify for !/ listener " + testone);
                pass = false;
            }
