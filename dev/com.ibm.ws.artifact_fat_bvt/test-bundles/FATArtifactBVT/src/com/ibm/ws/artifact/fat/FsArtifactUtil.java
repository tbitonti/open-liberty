...
public class FsArtifactUtil {
    ...
    public static boolean compare(ArtifactContainer c, Fs f, PrintWriter out) {
        boolean result = true;
    ...
    private static boolean traversalCompare(ArtifactContainer c, Fs f, PrintWriter out) {
        boolean result = true;
    ...
    /**
     * 
     * @param test boolean check, returned as result of method
     * @param e
     * @param f
     * @param out
     * @param reason message to output if test is false
     * @return value of test
     */
    private static boolean check(boolean test, EnclosedEntity e, Fs f, PrintWriter out, String reason) {
        counter++;
        if (!test) {
    ...
    private static boolean pathCheck(String path, Fs f, PrintWriter out) {
        boolean result = true;
        if (path == null) {
            result = false;
    ...
    private static boolean compareEntityInfo(boolean eIsContainer, EnclosedEntity e, Fs f, PrintWriter out) {
        return compareEntityInfo(eIsContainer, e, f, out, true);
    }

    private static boolean compareEntityInfo(boolean eIsContainer, EnclosedEntity e, Fs f, PrintWriter out, boolean processDownward) {
        boolean result = true;
    ...
    private static boolean checkParentPath(ArtifactContainer c, Fs f, PrintWriter out) {
        boolean result = true;

        Fs parent = f.getParent();
        ...
        if (!f.isRoot() && parent != null && !parent.isRoot()) {
            ...
            if (parent.getChildren() != null && parent.getChildren().length > 1) {
                Fs chosen = null;
                for (Fs child : parent.getChildren()) {
            ...
            Fs superparent = parent.getParent();
    ...
    private static boolean compareChildren(ArtifactContainer c, Fs parent, Fs[] children, PrintWriter out) {
        boolean result = true;
        ...
        for (ArtifactEntry e : c) {
            ...
            if (e == null) {
                ...
                if (namesWanted.contains(entryName)) {
                    ...
                    Fs matching = parent.getChildByName(entryName);
    ...
    private static boolean checkNameAndPath(boolean eIsContainer, EnclosedEntity e, Fs f, PrintWriter out) {
        boolean result = true;
    ...
    private static boolean checkResource(ArtifactEntry e, Fs f, PrintWriter out) {
        boolean result = true;
    ...
    private static boolean checkURLs(ArtifactContainer container, Fs f, PrintWriter out) {
        boolean result = true;
    ...
    private static boolean upwardNavigationCheck(boolean eIsContainer, EnclosedEntity e, Fs f, PrintWriter out) {
        boolean result = true;
        //makes sure for every entity, that the parent chain has entries with 
        //the expected name & path right up to absolute root.
        //start with the ones we have, to check mismatches there..
        //out.println("\nUpwards nav check for node " + f.getDebugPath());
        Fs parentFs = f;
        EnclosedEntity parentE = e;
        boolean thisEntityIsContainer = eIsContainer;
        //we stop when there is no parent defined in our test data, e.g. absolute root.
        while (parentFs != null && parentE != null) {
            boolean match = (parentFs == null && parentE == null) || (parentFs != null && parentE != null);

            //            if (parentE != null && parentFs != null)
            //                out.println("Comparing " + parentE.getPath() + " with " + parentFs.getDebugPath());

            result &= check(match, e, f, out,
                                 "Error while walking parent chain for entry at path " + f.getDebugPath() + " " + parentFs + " " + parentE);

            result &= checkNameAndPath(thisEntityIsContainer, parentE, parentFs, out);

            //move up a level.
            parentFs = parentFs.getParent();
            parentE = parentE.getEnclosingContainer();
            thisEntityIsContainer = true;
        }
        if ((parentFs == null && parentE != null) || (parentFs != null && parentE == null)) {
            if (parentE != null) {
                out.println("FAIL: upwards navigation check for " + f.getDebugPath() + " failed, entry still has unexpected parent of " + parentE.getName() + " at "
                            + parentE.getPath() + " parent check was originally for node " + f.getDebugPath());
            }
            if (parentFs != null) {
                out.println("FAIL: upwards navigation check for " + f.getDebugPath() + " failed, entry reported it had no parent at " + parentFs.getPath()
                            + " when parent was expected. Parent check was originally for node " + f.getDebugPath());
            }
