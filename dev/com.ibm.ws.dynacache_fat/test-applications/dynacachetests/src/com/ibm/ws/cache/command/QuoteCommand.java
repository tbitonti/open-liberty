...
public class QuoteCommand extends CacheableCommandImpl implements Quote {
    ...
    static {
        try {
            DynamicCacheAccessor.getCache().invalidateByTemplate(QuoteCommand.class.getName(), true);
        } catch (Exception ex) {
        }
    }
    ...
    public void setOutputProperties(TargetableCommand fromCommand) {
        ...
        if (!(fromCommand instanceof QuoteCommand)) {
            throw new IllegalStateException("TargetableCommand must be an instance of QuoteCommand");
        }
        QuoteCommand from = (QuoteCommand) fromCommand;
